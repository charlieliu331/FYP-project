...
from neural_punctuator.data.dataloader import BertTrainDataset, BertValDataset, collate, get_data_loaders, get_datasets_metrics
...
class BertPunctuatorTrainer(BaseTrainer):
    def __init__(self, model, preprocessor, config):
        ...
        if model_family == 'bert':
            ...
            exit(1)
        #TODO
        self.train_dataset, self.valid_dataset, self.space_count, self.p_count, self.q_count, self.comma_count, self.e_count = get_datasets_metrics(config)
        self.punct_count = self.space_count + self.p_count + self.q_count + self.comma_count + self.e_count
        ...
        # self.train_dataset, self.valid_dataset, self.space_count, self.p_count, self.comma_count = get_datasets_metrics(config)
        # self.punct_count = self.space_count + self.p_count + self.comma_count
    ...
    def mask_out(self,text,targets):
        ...
        exclaimation = self._preprocessor._tokenizer.encode("!")[1:-1]
        mask_token_number = self._preprocessor._tokenizer.encode(self.mask_token)[1:-1]
        
        space_placeholder = 250004
        ...
        mask |=  ((text == exclaimation) & (np.random.rand(*text.shape) < 1- self.e_count / self.punct_count))
    ...
    def full_mask_out(self,text,targets):
        ...
        exclaimation = self._preprocessor._tokenizer.encode("!",add_special_tokens=False)[0]
        ...
        values_mask = [period,comma,chinese_period,space_placeholder,question,exclaimation]
    ...
    def train(self):
        ...
        for epoch_num in range(self._config.trainer.num_epochs):
            ...
            for data in pbar:
                ...
                mask_tokens = torch.tensor(text == self.mask_token_number).to("cpu").bool()
            ...
            print("error counter:",err_counter)
            metrics = self.validate(printer_counter)
