...
class BertMetrics:
    def __init__(self, prefix, config):
        ...
        with open(config.data.train_data_path + prefix + "_data.pkl", 'rb') as f:
            #texts, targets, space_count, p_count, q_count, comma_count = pickle.load(f)
            texts, targets, space_count, p_count, q_count, comma_count, e_count = pickle.load(f)
    ...
    def get_metrics(self):
        ...
        return self.space_count, self.p_count, self.q_count, self.comma_count, self.e_count
...
class BertTrainDataset(Dataset):
    def __init__(self, prefix, config, is_train=False):
        ...
        with open(config.data.train_data_path + prefix + "_data.pkl", 'rb') as f:
            ...
            texts, targets, _,_,_,_,_ = pickle.load(f)
...
class BertValDataset(Dataset):
    def __init__(self, prefix, config, is_train=False):
        ...
        #with open(config.data.data_path + prefix + "_data.pkl", 'rb') as f:
        with open(config.data.val_data_path + prefix + "_data.pkl", 'rb') as f:
            #texts, targets, _,_,_,_ = pickle.load(f)
            texts, targets, _,_,_,_,_ = pickle.load(f)
            self.encoded_texts = [word for t in texts for word in t]
            self.targets = [t for ts in targets for t in ts]
...
def get_datasets_metrics(config):
    ...
    space_count, p_count, q_count, comma_count, e_count = BertMetrics("train",config).get_metrics()
    train_dataset = BertTrainDataset("train", config, is_train=True)
    if config.data.eval_type == 'valid':
        valid_dataset = BertValDataset("valid", config)
        logging.info("Using valid dataset for eval dataset.")
    elif config.data.eval_type == 'tst':
        valid_dataset = BertValDataset("test", config)
        logging.info("Using test dataset for eval dataset.")
    else:
        valid_dataset = BertValDataset("valid", config)
        logging.info("Eval dataset set default to valid dataset instead of test set.")
    return train_dataset, valid_dataset, space_count, p_count, q_count, comma_count,e_count
