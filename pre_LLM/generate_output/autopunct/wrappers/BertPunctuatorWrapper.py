...
class BertPunctuatorWrapper(BaseWrapper):
    ...
    def predict(self,message, ptype='all'):
        ...
        prediction = np.exp(prediction.detach().cpu().numpy()) #apply softmax manually without the following normalization step
        prediction = prediction.reshape(-1,3) #prediction = prediction.reshape(-1,4)
        prediction = prediction.argmax(-1) #This line takes the reshaped prediction array and applies argmax along the last dimension (-1), effectively finding the index of the maximum value in each row (or sub-array). Since the array has been reshaped such that each row corresponds to the set of class probabilities for a particular token, argmax(-1) determines the most likely class for each token. This step is the essence of classification: deciding which category (class) each token is most likely to belong to based on the model's predictions.

        punctArray = [' ','.',','] if ptype == 'all' else [' ','.',' ',' '] #punctArray = [' ','.','?',','] if ptype == 'all' else [' ','.',' ',' ']
